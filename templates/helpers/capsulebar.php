<?php
	...
	foreach($this->items as $key => $item) {
		$keys = explode(',',$key);  // more than one action can be bound to an item
...
<ol class="capsulebar">
	...
	<li class="item <?php echo $item['class']; ?><?php if($key === $activeKey) echo ' active'; ?><?php if($i == 0) echo ' first' ?><?php if($i == sizeof($this->items)-1) echo ' last' ?>">
		<a href="<?php echo $item['url']; ?>" class="capsule" accesskey="<?php echo $i+1; ?>" id="capsulebar-<?php $keys = explode(',', $key); echo $keys[0] ?>">
