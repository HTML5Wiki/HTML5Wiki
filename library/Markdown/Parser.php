 ...
 * Default level for headers added to the parsed level.
 * 
 */
if (!defined('MARKDOWN_HEADER_INITIAL_LEVEL')) {
	define('MARKDOWN_HEADER_INITIAL_LEVEL', 1);
}
...
# Terrible hack to check we haven't found an empty list item.
		...
		$level = ($matches[2]{0} == '=' ? 1 : 2) + MARKDOWN_HEADER_INITIAL_LEVEL;
		$block = "<h$level>" . $this->runSpanGamut($matches[1]) . "</h$level>";
		return "\n" . $this->hashBlock($block) . "\n\n";
	}

	function _doHeaders_callback_atx($matches) {
		$level = strlen($matches[1]) + MARKDOWN_HEADER_INITIAL_LEVEL;
